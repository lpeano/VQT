            ...
            if "already open" in str(e) or "consistency" in str(e):
                ...
                if clear_hdf5_consistency_flags(file_data_path):
                    ...
                    try:
                        test_handle = h5py.File(file_data_path, 'r', libver='latest')
                        test_handle.close()
                        print(f"[AUTO-REPAIR] ✓ Riparazione completata. Playback pronto.")
                    except:
                        print(f"[AUTO-REPAIR] ✗ Riparazione fallita. File ancora corrotto.")
                        sys.exit(1)
                else:
...
def update(frame, target_file_handle=None):
    ...
    if args.playback:
        ...
        if animazione_in_esecuzione:
            ...
            delta_lambda = 0.2  # Incremento parametro affine (2x velocità evoluzione)
